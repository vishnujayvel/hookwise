...
hookwise is a platform for guarding and observing Claude Code sessions.

## Why hookwise Exists

Claude Code is extraordinary. It can refactor an entire module, set up infrastructure, write tests -- all while you are reading the previous diff. That power is exactly why it needs guard rails and observability.
...
hookwise replaces all of that with a platform: declarative guards, analytics that tell you where your time actually goes, insights that surface friction in your workflow, feeds that surface context in your status line, recipes that let you share configurations, and a TUI that ties it all together.

## Core Principles

### 1. Platform, Not Just Config

hookwise started as a YAML config layer for guard rails. It is now a platform with a SQLite analytics engine, a cache bus, a feed daemon, a TUI, and a recipe system. YAML remains the right format for declaring guards and feed settings -- the things that should be human-readable, git-tracked, and declarative. But YAML is not the whole platform.

The platform needs a data layer, not just a config layer. Recipes need a standard way to persist structured data. Analytics needs a durable store. Without a standard data layer, every recipe author invents their own storage, every new feature adds another ad hoc file, and the platform fragments.
...
This principle applies specifically to the guard system. Other parts of hookwise -- analytics dashboards, workflow insights, feed-backed status lines, the TUI, recipe ecosystems -- can and should be rich, interactive, and engaging. The platform grows; guards stay boring.

### 5. Recipes Need a Data Contract

A recipe is a shareable hookwise configuration: guards, feed producers, status line segments. Today, a recipe is a directory with `hooks.yaml`. That is enough for declarative configuration but not enough for recipes that need to persist data.
